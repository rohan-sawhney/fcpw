...
// BVH type identifiers
enum CUDABvhType {
    CUDA_UNDEFINED_BVH = 0,
    CUDA_LINE_SEGMENT_BVH = 1,
    CUDA_TRIANGLE_BVH = 2,
    CUDA_LINE_SEGMENT_SNCH = 3,
    CUDA_TRIANGLE_SNCH = 4
};
...
public:
    ...
    // updateObjectVertex in the Scene class, or directly in GPU code (set updateGeometry
    // to false if the geometry is updated directly on the GPU).
...
private:
    // members
    cuda::CUDAContext context;
    cuda::CUDABvhBuffers bvhBuffers;
