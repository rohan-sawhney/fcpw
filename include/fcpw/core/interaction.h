...
struct Interaction {
	// constructor
	Interaction(): p(zeroVector<DIM>()), n(zeroVector<DIM>()), uv(zeroVector<DIM - 1>()),
				   d(maxFloat), sign(0), primitiveIndex(-1), nodeIndex(-1),
				   referenceIndex(-1), objectIndex(-1), distanceInfo(DistanceInfo::Exact) {}
