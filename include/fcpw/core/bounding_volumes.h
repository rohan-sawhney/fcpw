...
struct BoundingBox {
    ...
    bool intersectRobust(const Ray<DIM>& r, const RobustIntersectionData<DIM>& rid,
        ...
        if constexpr (DIM == 3) {
            ...
            float boxBounds[6] = { pMin[0], pMin[1], pMin[2], pMax[0], pMax[1], pMax[2] };
            Vector3 pMinPerm{boxBounds[rid.near[0]], boxBounds[rid.near[1]], boxBounds[rid.near[2]]};
            Vector3 pMaxPerm{boxBounds[rid.far[0]], boxBounds[rid.far[1]], boxBounds[rid.far[2]]};
