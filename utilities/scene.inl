...
inline void Scene<DIM>::loadFiles(bool computeWeightedNormals, bool randomizeObjectTransforms)
{
	...
	// output sum of primitives if silenced (awkward var name)
	if (isQuiet){
		int totalPrims = 0;
		for (int i = 0; i < nFiles; i++){
			totalPrims += objects[i].size();
		}
		std::cout << totalPrims << ",";
	}
}
...
inline std::shared_ptr<Aggregate<DIM>> makeAggregate(const AggregateType& aggregateType,
													 std::vector<std::shared_ptr<Primitive<DIM>>>& primitives, bool silenceOutput)
{
	int leafSize = 4;
	if (aggregateType == AggregateType::Bvh_LongestAxisCenter) {
		return std::make_shared<Bvh<DIM>>(primitives, CostHeuristic::LongestAxisCenter, leafSize, silenceOutput);

	} else if (aggregateType == AggregateType::Bvh_SurfaceArea) {
		return std::make_shared<Bvh<DIM>>(primitives, CostHeuristic::SurfaceArea, leafSize, silenceOutput);

	} else if (aggregateType == AggregateType::Bvh_OverlapSurfaceArea) {
		return std::make_shared<Bvh<DIM>>(primitives, CostHeuristic::OverlapSurfaceArea, leafSize, silenceOutput);
...
inline void Scene<DIM>::buildAggregate(const AggregateType& aggregateType)
{
	...
	for (int i = 0; i < nObjects; i++) {
		objectAggregates[i] = makeAggregate<DIM>(aggregateType, objects[i], isQuiet);
	}
	...
	if (objectInstances.size() == 1) {
		...
		// make aggregate
		aggregate = makeAggregate<DIM>(aggregateType, objectInstances, isQuiet);
	}
}
...
inline void Scene<DIM>::buildEmbreeAggregate()
{
	...
	aggregate = std::make_shared<EmbreeBvh<DIM>>(objects[0], soups[0], isQuiet);
	objectInstances.clear();
}
#endif
